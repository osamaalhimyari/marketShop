  <div class="md:col-span-1 bg-white dark:bg-gray-800 p-6 mt-6  mr-2 ml-2 rounded-lg shadow-lg">
      ...
      <div class="flex  mb-5">
          ...
          <h5 class="summary-ProductsCount  font-bold" id="summary-ProductsCount"> {{ $order->carts->count() }}</h5>
      </div>
      <hr class="my-4">

      <div class="flex  mb-5">
          <h5 class="text-uppercase pl-10 font-bold">{{ __('totalPrice') }}:</h5>
          <h5 class="total-price  font-bold text-green-700" id="total-price">{{ $globalConfig->currency->sign }}
              {{ $order->total_price ?? 0.0 }}</h5>
      </div>
      <hr class="my-4">
      <div>
          <label class="block text-sm font-medium leading-6 text-gray-900 dark:text-white p-3 ">
              {{ __('fullname') }}</label>
          <div class="mt-2">
              <input type="text" value="{{ $order->name }}" disabled id="name" name="name"
      ...
      <div>
          ...
          <div class="mt-2">
              ...
              <input type="text" value="{{ $order->address }}" disabled multiple id="address" name="address"
