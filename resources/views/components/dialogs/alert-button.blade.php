...
<button type="button" onclick="event.stopPropagation(); toggleModal('{{ $id }}', true)"
    ...
    <!-- Modal content -->
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg text-center">
            ...
            <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-3">
                <form id="deleteForm-{{ $id }}"  name="deleteForm-{{ $id }}" method="POST" action="{{ $rute }}">
                    @csrf
                    @method('DELETE')
                <button type="submit" form="deleteForm-{{ $id }}"
                    onclick="event.stopPropagation(); toggleModal('{{ $id }}', false)"
                ...
                <button type="reset" form="deleteForm-{{ $id }}"
                    onclick="event.stopPropagation(); toggleModal('{{ $id }}', false)"
